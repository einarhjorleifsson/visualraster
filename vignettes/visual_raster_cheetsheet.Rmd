...
```{r echo = FALSE}
p1 <- lh(r_origin) + labs(subtitle = "r_origin")
...
## boundaries
```{r, echo = FALSE}
r_clus = init(r, function(x) ifelse(runif(x) > 0.1, 1, NA))
r_boundaries = raster::boundaries(r_clus)
p1 <- lh(r_clus) + labs(subtitle = "r_clus")
p2 <- lh(r_boundaries) + labs(subtitle = "boundaries(r_clus)")
p1 + p2
```
